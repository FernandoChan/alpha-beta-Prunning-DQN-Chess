...
def negamax(is_ai, depth, alpha, beta):
    ...
    blank_list = list(set(list_all).difference(set(list3)))#找出空白的点（list_all - list3）
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                if (last_pt[0] + i, last_pt[1] + j) in blank_list:
                    ...
                    #put the element into the first one
                    # x = [2, 5, 10]
                    # x.insert(2, 77)
                    # print(x)  # [2, 5, 77, 10]
...
def cal_score(m, n, x_decrict, y_derice, enemy_list, my_list, score_all_arr):
    ...
    #score_all_arr only have 1 & 2 two rows
...
def main():
    ...
    while g == 0:
        ...
        if change % 2 == 1:
            pos = ai()
            #Get the position by the AI

            #All the nodes are occupied
            if pos in list3:
